...
try:
	scor=0
	b=pd.read_csv(os.getcwd()+"/PoBT-master/output/hits/HitsHSF1.gff",delimiter='\t',header=None)
	...
	b=0
	scor=0
with open(os.getcwd()+"/Best_Species/current.csv",'r') as f2:
	ss=f2.readlines()
...
if l1[-1]!="Tf_score":
	...
	with open(os.getcwd()+"/Best_Species/current.csv", "w") as f3:
		for i in l1:
			f3.write(i)
			if (i==l1[-1]):
				f3.write("\n")
